...
export class GithubButtonComponent {
	...
	constructor(
		...
		this.gitAccessCodeObserver = new Observable((observer: Observer<string | null>) => {
		...
		this.gitTokenHandlerObserver = new Observable((observer: Observer<Object>) => {
			this.gitAccessCodeObserver.subscribe((code: string | null) => {
				...
				this.accessToken$.subscribe((tokenResponse: any) => {
					if (tokenResponse.error) {
						this.toastr.error(tokenResponse.error_description, tokenResponse.error.replace(/[_]/g, ' '))
	...
	ngOnInit(): void {
		...
		new Observable((observer: any) => {
			this.gitTokenHandlerObserver.subscribe((token: any) => {
				const userProfileInfo$ = this.config.fetchGitUserProfileFromToken(token.token_type, token.access_token);
				const userEmails$ = this.config.fetchGitUserEmailsFromToken(token.token_type, token.access_token);
			...
			if (this.gitUserData! && this.gitUserEmails!) {
				const gitAuthModel: UserAuthModel = this.config.transformUserDataToModel(this.gitUserData, this.gitUserEmails, this.btnType);
